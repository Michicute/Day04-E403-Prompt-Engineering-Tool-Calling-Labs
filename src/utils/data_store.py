...
class OrderDataStore:
    """Local catalog, deterministic discount, pricing, and order persistence store."""
            ...
            results.append(
                (
                    ...
                    product.product_id,
                    {
                        ...
                        "next_step": "Call get_product_details with chosen product IDs before pricing or saving.",
                    },
                )
            )
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        """Return exact product details plus a deterministic validation token."""
        details: list[dict] = []
        for product_id in product_ids:
            product = self.product_index.get(product_id)
            if not product:
                details.append({"status": "not_found", "product_id": product_id})
                continue
            details.append(
                {
                    "status": "ok",
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        """Return a deterministic simulated campaign discount."""
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        """Validate stock/token and calculate subtotal, discount, and final total."""
        if not items:
            return {"status": "error", "errors": ["Order must contain at least one line item."]}
    ...
    def save_order(
        ...
        *,
        ...
        """Recompute totals, persist the final order JSON, and return its payload/path."""
